...
import { updateColor } from "../../api/investAPI";
...
export const UpdateCover = () => {
  ...
  const { accountNumber, accountTitle, color } = location.state as IState;
  ...
  const update = async () => {
    const updateResponse = await updateColor(myColor, token);
  ...
  return (
    <div className="h-full pt-6 px-4 bg-white flex flex-col justify-between">
      <div className="space-y-5">
        <TopBar pre={"/mypage/account"} title={""} skip={""} />
